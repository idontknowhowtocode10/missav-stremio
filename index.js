...
// ─── Debug route (visit /debug to see what prefix is being used) ─────────────
app.get('/debug', async (_req, res) => {
  const { fetchCatalog } = require('./src/scraper');
  const NodeCache = require('node-cache');
  try {
    const { discoverPrefix } = require('./src/scraper');
    // Re-import to get prefix
    const scraper = require('./src/scraper');
    res.json({
      message: 'Trigger a catalog fetch first, then check logs',
      test: 'Visit /catalog/movie/missav-new.json to trigger a fetch',
    });
  } catch (e) {
